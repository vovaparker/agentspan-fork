...
/**
 * Validates the post-loop output synthesizer that ensures the agent's
 * workflow ``result`` is non-empty even when the loop terminated on a
 * TOOL_CALLS turn (the {@code stop_when} fired right after the model
 * called a writer tool, leaving the LLM's text result empty).
 *
 * <p>Without this synthesis, the explorer agent's output is {@code "[]"}
 * and the downstream stage sees nothing — the bug the user reported on
 * workflow {@code 420d4c2f-...}. With it, the workflow output carries a
 * JSON dump of the last turn's tool-call inputs, surfacing the
 * {@code content} arg of {@code write_coder_plan} et al.</p>
 */
class SynthOutputScriptTest {
    ...
    private String run(String inputJson) {
        String wrapped = "var $ = " + inputJson + "; var __r = " + SCRIPT + "; JSON.stringify({result: __r});";
    ...
    void summarizesMultipleToolCallsInOneTurn() {
        String input = "{\"llm_result\": null, \"tool_calls\": ["
                + "{\"name\": \"write_coder_plan\", \"inputParameters\": {\"content\": \"plan\"}},"
