...
/**
 * Unit tests for {@link PlannerContextFetchTask} — pins the cache + ETag
 * behaviour /dg #4 introduced. No network — HttpClient is mocked.
 */
class PlannerContextFetchTaskTest {
    ...
    private static HttpResponse<String> stubResponse(int status, String body, String etag) {
        ...
        java.net.http.HttpHeaders headers = etag == null
                ? java.net.http.HttpHeaders.of(Map.of(), (a, b) -> true)
                : java.net.http.HttpHeaders.of(Map.of("ETag", java.util.List.of(etag)), (a, b) -> true);
    ...
    /**
     * Helper to stub {@code HttpClient.send} which has a generic return
     * type Mockito's {@code when(...).thenReturn(...)} can't infer.
     * {@code doReturn} accepts {@code Object...} so it sidesteps the
     * inference issue.
     */
    private static void stubSend(HttpClient http, HttpResponse<String>... responses) throws Exception {
        Object first = responses[0];
        Object[] rest = new Object[responses.length - 1];
        for (int i = 1; i < responses.length; i++) rest[i - 1] = responses[i];
        doReturn(first, rest).when(http).send(any(HttpRequest.class), any(HttpResponse.BodyHandler.class));
    }
    ...
    void cacheHitOnSecondCallWithinTtlSkipsHttpClient() throws Exception {
        ...
        assertThat(resp2.get("body")).as("second call must return cached body").isEqualTo("doc body");
    ...
    void requiredFalseSurfacesNon2xxWithoutFailingTask() throws Exception {
        ...
        TaskModel t = taskWith(
                Map.of("url", "https://example.com/doc", "headers", Map.<String, String>of(), "required", false));
    ...
    void requiredTrueFailsTaskOn5xx() throws Exception {
        ...
        TaskModel t = taskWith(
                Map.of("url", "https://example.com/doc", "headers", Map.<String, String>of(), "required", true));
        task.start(null, t, null);
    ...
    void differentHeadersProduceDistinctCacheKeys() throws Exception {
        ...
        HttpClient http = mock(HttpClient.class);
        stubSend(http, stubResponse(200, "body-user-A", null), stubResponse(200, "body-user-B", null));
        PlannerContextFetchTask task = new PlannerContextFetchTask(http);

        TaskModel t1 = taskWith(Map.of(
                "url", "https://example.com/doc", "headers", Map.of("Authorization", "Bearer A"), "required", true));
        task.start(null, t1, null);
        TaskModel t2 = taskWith(Map.of(
                "url", "https://example.com/doc", "headers", Map.of("Authorization", "Bearer B"), "required", true));
        task.start(null, t2, null);
