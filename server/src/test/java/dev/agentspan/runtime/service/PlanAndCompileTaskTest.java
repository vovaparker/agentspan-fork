 ...
 *
 * <p>Each test drives {@code task.start(workflow, taskModel, executor)}
 * directly — the task does not touch the workflow or executor, so passing a
 * fresh {@link WorkflowModel} and {@code null} executor is sufficient.
 */
class PlanAndCompileTaskTest {
    ...
    private String compilePlanExpectError(String planJson) {
        ...
        Object error = output.get("error");
        if (error == null) {
            throw new AssertionError("Expected compile error but got workflowDef: " + output.get("workflowDef"));
        }
    ...
    private void collectTasks(List<Map<String, Object>> tasks, List<Map<String, Object>> out) {
        ...
        for (Map<String, Object> t : tasks) {
            ...
            if ("FORK_JOIN".equals(type)) {
                List<List<Map<String, Object>>> forkTasks = (List<List<Map<String, Object>>>) t.get("forkTasks");
                ...
                if (decisionCases != null) decisionCases.values().forEach(branch -> collectTasks(branch, out));
                List<Map<String, Object>> defaultCase = (List<Map<String, Object>>) t.get("defaultCase");
    ...
    void testNoSuccessConditionUsesDefaultPassCheck() {
        ...
        boolean hasDefaultEvalTask = tasks.stream()
                ...
                .anyMatch(t -> {
                    ...
                    Map<String, Object> inputs = (Map<String, Object>) t.get("inputParameters");
                    if (inputs == null) return false;
                    return String.valueOf(inputs.getOrDefault("expression", "")).contains("passed");
                });
    ...
    void testMultipleValidationsUseForkJoin() {
        ...
        Map<String, Object> forkTask = topTasks.stream()
                ...
                .orElseThrow();
        @SuppressWarnings("unchecked")
        List<List<Map<String, Object>>> forkTasks = (List<List<Map<String, Object>>>) forkTask.get("forkTasks");
    ...
    void testSuccessConditionAllowsLiteralBannedWordInString() {
        String[] safeWithLiterals = {
            "$.kind === 'constructor'", "$.role !== 'eval-pending'", "$.msg === 'Function returned ok'",
        };
    ...
    void testValidationSwitchFailsClosed() {
        ...
        boolean defaultHasTerminate = defaultCase.stream().anyMatch(t -> "TERMINATE".equals(t.get("type")));
    ...
    void testDefaultTimeoutWhenHarnessTimeoutAbsent() {
        Map<String, Object> wf =
                compilePlan("{ \"steps\": [{\"id\": \"s1\", \"operations\": [{\"tool\": \"noop\", \"args\": {}}]}] }");
    ...
    void testRejectsUnknownToolName() {
        ...
        Map<String, Object> output = runWithKnownTools(planJson, null, List.of("read_file", "write_file"));
        Object error = output.get("error");
    ...
    void testAcceptsKnownToolName() {
        ...
        Map<String, Object> output = runWithKnownTools(planJson, null, List.of("str_replace", "read_file"));
    ...
    void testToolWithoutGuardrailsEmitsBareSimple() {
        ...
        Map<String, Object> output = runWithParentTools(planJson, null, List.of("do_thing"), List.of(bareTool));
        assertThat(output.get("error")).isNull();
        @SuppressWarnings("unchecked")
        Map<String, Object> wf = (Map<String, Object>) output.get("workflowDef");
        @SuppressWarnings("unchecked")
    ...
    void testToolWithRegexGuardrailEmitsGate() {
        ...
        Map<String, Object> output = runWithParentTools(planJson, null, List.of("run_query"), List.of(guardedTool));
        assertThat(output.get("error")).isNull();
        @SuppressWarnings("unchecked")
        Map<String, Object> wf = (Map<String, Object>) output.get("workflowDef");
        ...
        boolean hasFormat = all.stream()
                .anyMatch(t -> "INLINE".equals(t.get("type"))
                        && String.valueOf(t.get("taskReferenceName")).contains("_format"));
        assertThat(hasFormat).as("expected format INLINE for guardrail content").isTrue();

        // Regex guardrail INLINE present.
        boolean hasRegex = all.stream()
                .anyMatch(t -> "INLINE".equals(t.get("type"))
                        && String.valueOf(t.get("taskReferenceName")).contains("regex_guardrail"));
        ...
        Map<String, Object> gate = all.stream()
                .filter(t -> "SWITCH".equals(t.get("type"))
                        && String.valueOf(t.get("taskReferenceName")).contains("guardrail_gate"))
                .findFirst()
                .orElseThrow(() -> new AssertionError("expected SWITCH gate for guardrail; got types: "
                        + all.stream().map(t -> t.get("type")).toList()));
        ...
        boolean terminates = cases.get("raise").stream().anyMatch(t -> "TERMINATE".equals(t.get("type")));
        assertThat(terminates)
                .as("the configured case must TERMINATE in plan mode")
                .isTrue();
        ...
        boolean simpleInDefault =
                defaultCase.stream().anyMatch(t -> "SIMPLE".equals(t.get("type")) && "run_query".equals(t.get("name")));
    ...
    void testGuardrailGateEmitsOnlyConfiguredCase_perOnFailMode() {
        ...
        for (var entry : expected.entrySet()) {
            ...
            Map<String, Object> output = runWithParentTools(planJson, null, List.of("do_thing"), List.of(guardedTool));
            assertThat(output.get("error"))
                    .as("compile error for on_fail=" + onFail)
                    .isNull();
            @SuppressWarnings("unchecked")
            Map<String, Object> wf = (Map<String, Object>) output.get("workflowDef");

            Map<String, Object> gate = allTasks(wf).stream()
                    .filter(t -> "SWITCH".equals(t.get("type"))
                            && String.valueOf(t.get("taskReferenceName")).contains("guardrail_gate"))
                    .findFirst()
                    .orElseThrow(() -> new AssertionError("expected guardrail_gate SWITCH for on_fail=" + onFail));

            @SuppressWarnings("unchecked")
            Map<String, List<Map<String, Object>>> cases =
                    (Map<String, List<Map<String, Object>>>) gate.get("decisionCases");
            assertThat(cases.keySet())
                    .as("on_fail=%s should emit exactly %s, got %s", onFail, entry.getValue(), cases.keySet())
    ...
    void testMultipleGuardrailsChainSequentially() {
        ...
        Map<String, Object> output = runWithParentTools(planJson, null, List.of("do_thing"), List.of(toolCfg));
        assertThat(output.get("error")).isNull();
        @SuppressWarnings("unchecked")
        Map<String, Object> wf = (Map<String, Object>) output.get("workflowDef");
        List<Map<String, Object>> all = allTasks(wf);

        long switchGates = all.stream()
                .filter(t -> "SWITCH".equals(t.get("type"))
                        && String.valueOf(t.get("taskReferenceName")).contains("guardrail_gate"))
                .count();
    ...
    void testDefaultRetryGuardrailStillBlocksSimple() {
        ...
        List<Map<String, Object>> top = (List<Map<String, Object>>) wf.get("tasks");
        boolean simpleAtTop =
                top.stream().anyMatch(t -> "SIMPLE".equals(t.get("type")) && "do_thing".equals(t.get("name")));
        assertThat(simpleAtTop)
                .as("SIMPLE must NOT be a top-level sibling of the gate; nesting in defaultCase "
        ...
        List<Map<String, Object>> defaultCase = (List<Map<String, Object>>) gate.get("defaultCase");
        boolean simpleInDefault =
                defaultCase.stream().anyMatch(t -> "SIMPLE".equals(t.get("type")) && "do_thing".equals(t.get("name")));
    ...
    void testWorkerToolOpEmitsSimple_regression() {
        ...
        Map<String, Object> output = runWithParentTools(planJson, null, List.of("do_thing"), List.of(tc));
        assertThat(output.get("error")).isNull();
        @SuppressWarnings("unchecked")
        Map<String, Object> wf = (Map<String, Object>) output.get("workflowDef");
        List<Map<String, Object>> all = allTasks(wf);
    ...
    void testAgentToolOpEmitsSubWorkflow() {
        ...
        Map<String, Object> output = runWithParentTools(planJson, null, List.of("subtask_coder"), List.of(tc));
        assertThat(output.get("error")).isNull();
        @SuppressWarnings("unchecked")
        Map<String, Object> wf = (Map<String, Object>) output.get("workflowDef");
        List<Map<String, Object>> all = allTasks(wf);
    ...
    void testMcpToolOpEmitsCallMcpTool() {
        ...
        Map<String, Object> output = runWithParentTools(planJson, null, List.of("search_docs"), List.of(tc));
        assertThat(output.get("error")).isNull();
        @SuppressWarnings("unchecked")
        Map<String, Object> wf = (Map<String, Object>) output.get("workflowDef");
        List<Map<String, Object>> all = allTasks(wf);
        Map<String, Object> task = all.stream()
                ...
                .findFirst()
                .orElseThrow(() -> new AssertionError("expected CALL_MCP_TOOL task for mcp op"));
    ...
    void testHttpToolOpEmitsHttp() {
        ...
        Map<String, Object> output = runWithParentTools(planJson, null, List.of("post_message"), List.of(tc));
        assertThat(output.get("error")).isNull();
        @SuppressWarnings("unchecked")
        Map<String, Object> wf = (Map<String, Object>) output.get("workflowDef");
        List<Map<String, Object>> all = allTasks(wf);
    ...
    void testUnknownToolTypeFallsBackToSimple() {
        ...
        Map<String, Object> tc = Map.of("name", "exotic_thing", "toolType", "experimental_new_type");
        Map<String, Object> output = runWithParentTools(planJson, null, List.of("exotic_thing"), List.of(tc));
        assertThat(output.get("error")).isNull();
        @SuppressWarnings("unchecked")
        Map<String, Object> wf = (Map<String, Object>) output.get("workflowDef");
        List<Map<String, Object>> all = allTasks(wf);
    ...
    void testGenerateOpWithAgentToolEmitsSubWorkflow() {
        ...
        Map<String, Object> sub = all.stream()
                .filter(t -> "SUB_WORKFLOW".equals(t.get("type")))
                .findFirst()
                .orElseThrow(() -> new AssertionError("generate-op with agent_tool must emit SUB_WORKFLOW"));
        assertThat(sub.get("name")).isEqualTo("subtask_coder_agent_wf");
        @SuppressWarnings("unchecked")
        ...
        assertThat(String.valueOf(inputs.get("prompt"))).startsWith("${").contains(".output.result.request");
    }
    ...
    void testGenerateOpWithMcpEmitsCallMcpTool() {
        ...
        Map<String, Object> wf = (Map<String, Object>) output.get("workflowDef");
        List<Map<String, Object>> all = allTasks(wf);
        Map<String, Object> mcp = all.stream()
                .filter(t -> "CALL_MCP_TOOL".equals(t.get("type")))
                .findFirst()
                .orElseThrow(() -> new AssertionError("generate-op with mcp must emit CALL_MCP_TOOL"));
        @SuppressWarnings("unchecked")
    ...
    void testAgentToolRetryOverrideFromConfigPropagates() {
        ...
        tc.put("config", cfg);
        Map<String, Object> output = runWithParentTools(planJson, null, List.of("flaky_agent"), List.of(tc));
        assertThat(output.get("error")).isNull();
        @SuppressWarnings("unchecked")
        Map<String, Object> wf = (Map<String, Object>) output.get("workflowDef");
        Map<String, Object> sub = allTasks(wf).stream()
                .filter(t -> "SUB_WORKFLOW".equals(t.get("type")))
                .findFirst()
                .orElseThrow();
    ...
    void testCompileIsDeterministicAcrossInvocations() throws Exception {
        ...
        List<String> names = List.of("coder", "doc_search", "publish", "approve", "check_word_count");
    ...
    void testAgentToolValidationOpEmitsSubWorkflow() {
        ...
        Map<String, Object> output =
                runWithParentTools(planJson, null, List.of("do_thing", "judge"), List.of(worker, judge));
        assertThat(output.get("error")).isNull();
        @SuppressWarnings("unchecked")
        Map<String, Object> wf = (Map<String, Object>) output.get("workflowDef");
        List<Map<String, Object>> all = allTasks(wf);
