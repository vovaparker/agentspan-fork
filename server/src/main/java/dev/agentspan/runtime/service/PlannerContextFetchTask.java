...
/**
 * /dg #4: HTTP fetch task for PLAN_EXECUTE {@code plannerContext} URL
 * entries with an in-process TTL cache and {@code If-None-Match}
 * conditional-GET support.
 *
 * <p>Previously the compiler emitted a plain Conductor {@code HTTP} task
 * per URL — every planner invocation made a fresh GET regardless of
 * whether the doc had changed since the last run. On a hot pipeline
 * (dozens of plans/minute) that's dozens of identical GETs/minute
 * against the upstream doc CMS. A doc-host outage stalled every plan
 * for the full read timeout, sequentially per URL.
 *
 * <p>This task:
 * <ul>
 *   <li>Caches responses in-process keyed on {@code (url, sorted-headers)}
 *       with a per-entry TTL (default 60s).</li>
 *   <li>Sends {@code If-None-Match} when a previous {@code ETag} is in
 *       cache; a 304 refreshes the cache TTL without re-downloading the
 *       body.</li>
 *   <li>Bounded cache: ~1000 entries with LRU eviction (LinkedHashMap
 *       access-order). Sufficient for typical N≤dozens-of-distinct-URLs
 *       workloads — adjust upward if telemetry shows high evict rate.</li>
 *   <li>Surfaces {@code cache_hit} on the output so observability can
 *       distinguish fresh fetches from cached returns.</li>
 * </ul>
 *
 * <p>The output shape mirrors Conductor's built-in HTTP task —
 * {@code response.body}, {@code response.statusCode} — so the
 * downstream {@code _ctx_build} INLINE (which reads
 * {@code ${fetchRef.output.response.body}}) works without changes.
 *
 * <p>Concurrency: cache is a {@code synchronized} access-order
 * {@code LinkedHashMap}. Lock is held only for the get + size-bound
 * check; HTTP I/O happens outside any lock. Cache reads are O(1).
 */
public class PlannerContextFetchTask extends WorkflowSystemTask {
    ...
    /**
     * Cache entry. {@code expiresAtMillis} is checked on read; entries
     * past it are treated as misses (but the {@code etag} survives so a
     * conditional GET can revalidate).
     */
    ...
    /**
     * Access-order LinkedHashMap with size-bounded eviction. The
     * {@code synchronized} wrapper makes it safe to call from concurrent
     * task workers; held only for cache get/put, never across HTTP I/O.
     */
    ...
    PlannerContextFetchTask(HttpClient httpClient) {
        ...
        this.cache = Collections.synchronizedMap(new LinkedHashMap<String, Entry>(64, 0.75f, true) {
            @Override
    ...
    public void start(WorkflowModel workflow, TaskModel task, WorkflowExecutor executor) {
        ...
        try {
            ...
            String newEtag = resp.headers().firstValue("ETag").orElse(hit != null ? hit.etag : null);
